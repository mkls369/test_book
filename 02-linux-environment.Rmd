...
When working on large projects and big datasets you will likely encounter distrbuted computing 
running onunix like system which usually means a linux distribution or a version MacOS. 
Cloud computing services like Amazon's EC2, Google's Compute Engine work on linux Virtual Machines 
so you will have to have some linux knowledge to leverage them. 
Most importantly, when working with a technology you are not familiar with you will likely stumble upon a 
post, tutorial or a course which will most likely use linux/OSX. 

Also it is worth mentioning that most linux distributions(variaties of linux operating system) are
free to use. On top of the a lot of free, open-source is available to linux users.

Last but not least since linux is open-source it is highly customizable to your needs.

This chapter will get you acquainted with linux environment if ou are on a Mac you should be able to 
follow along with minimal code changes. 

## The command line. Terminology.

If you are running linux you should be able to search for 'terminal' to start a terminal session. If
you are working on Windows an easy way to emulate an Ubuntu terminal is by installing 
[Windows Subsystem for Linux](https://docs.microsoft.com/en-us//wsl/install-win10). 

First let's get some terminology out of the way. You will often hear these words when referring to the
black (or dark colored) screen with a blinking cursor: terminal (emulator), console, command line, shell. 
Below are definitions from [The Linux Information Project](http://www.linfo.org/) (We will be 
introducing more terms along this book, it's a good habit of looking them up to avoid 
confusion)

*Terminal* - A terminal window, also referred to as a terminal emulator, is a text-only window in a 
graphical user interface (GUI) that emulates a console.

The original meaning of *console* as it related to computers was (1) an instrument panel containing 
the controls for a computer ... Another term for console as used in this sense is terminal.

These two terms have historical origins - you can read about them in their respective 
[linfo](http://www.linfo.org/) pages. In our case they are often used interchangeably to refer to
**program** that **emulates** a machine that is used to input data.

A *command line* is the space to the right of the command prompt on an all-text display mode on a 
computer monitor (usually a CRT or LCD panel) in which a user enters commands and data.

So the command line is the textual interface - a place for you to input commands for the computer to
execute.

A **shell** is a program that provides the traditional, text-only user interface for Linux and other 
Unix-like operating systems. Its primary function is to read commands ... and then execute (i.e., run) 
them.

Finally shell is the program that allows for easy human-computer communication - it relays commands 
to the deeper layers of the OS. In later Sections we will introduce commands that are executed by the shell.
One important thing to note is that shells are extremely customizable and many off-the-shelf varieties exist, 
we will be using **bash** because of its popularity and availability.

## The command line. Basic usage.

Now that we know that the commands line is we can use our terminal emulator start typing in commands for
our shell to execute (see it came together!). This section will introduce the main commands you will 
need in your daily routine. Note that you should be careful when not only deleting files, 
but also writing because your work will be overwritten.

### Navigating folders

I'm assuming you already installed linux or WSL if you are on Windows. The terminal shows the current 
folder. You should see a `~$` before your blinking cursor. Type `pwd` for print working directory
you should see `/home/your_username` printed after the command is executed. This is your home folder
the `~` aka **tilde** is use as a shortcut to your home directory.

To know what's in the current folder type `ls` (for list). It might be empty and nothing will get printed. 
Wecan go *up* one directory level to `/home` directory to check it out. Type `cd ..` in the command 
prompt to do so. It is equivalent to Alt + ↑ or clicking the up arrow in the File Explorer navigation bar.
...
Last thing to note is that you can navigate to a folder by absolute path like 
`cd /home/your_username/` and by relative path `cd your_username`.

### Creating folders and files

I'm assuming you returned to your folder `/home/your_username/` and are ready to make some folders i.e.
...
if our file has been created successfully. 

Let's break down what happened when you typed the `echo ...` command. 
...
being standard input (stdin) and standard error (stderr). These streams are plain text that can be
...
are indeed the default options) they can be any file or even device and the standard streams can be 
redirected as we will se below. This explanation was long-winded but it will be useful when 
discussing some of the commands.

The command `echo` writes the string "This is some text" to standard output. Let's try the command 
...
I mentioned there are several options to create files. I think you can already see the limitations of 
...
Let's discuss VIM first. Why VIM? It's quite popular according to a recent Stack Overflow 
[survey](https://insights.stackoverflow.com/survey/2019#development-environments-and-tools) and
it might be already available on your linux environment.
...
You can type in normal manner now. Write something in like: This is a new line written in VIM. 
Now 'save' the file. First you need return to to Command mode by hitting the **Esc** key 
...
The following commands will help you to do that:
...
**Ctrl** key to jump across individual words. Try various approaches and find the one that is convienient 
for you - you don't have to use something because someone claimed so without justifying the motivation. 
...
We are balck in the console. To delete a file simply use the `rm` command followed with the file name 
like `rm new_vim_file.txt`. Be careful when deleting files - the action is irreversible It is smart 
to select the files first before deleting them to make sure you are deleting you want to delete. 
This is especially important when deleting multiple files. 

First let's learn how to make multiple files. We will use `touch` command
...
with regex you should know what `*` and `?` stand for. If not `*` will match any symbol(s) i.e. one 
or more and `?` will match any single symbol (in this case symbols mean any character that is used
in the name) Let's try them out by listing our files: `ls file*.txt` should output all the file 
names we have created, It is very natural because the `*` (we can refer to it as **glob**) matches 
the first number for the files `file1.txt` to `file9.txt` and it also matches the 0 for `file10.txt`. 
But what if we used the question mark? `ls file?.txt` should show you only files `file1.txt` to 
`file9.txt` because it match a character a single time.

We know know how deletion will work and we're not afraid to use it. Let's typr `rm file*.txt`. You can see
the file `file10.txt` is the only one left. You could point that it was very convenient for us to leave 
...
You can also copy multiple files like this: `cp files?.txt another_directory/`.

Moving a file can be done with `mv`: `mv file_to_move new_location_of_file`. 
This command is also used for renaming files.

Copying/moving folders is achieved by the same command.
...
If your current computer is running  thus can be done by using [PuTTY](https://www.putty.org/), 
however I strongly encourage to get [WSL](https://docs.microsoft.com/en-us//wsl/install-win10). 
This way you can enjoy a Ubuntu-like terminal on you machine. This means the following instructions 
are relevant for both Windows on WSL and linux users.

Things to know when connecting to a remote computer:

1. Getting a linux account. Ask you administrator to create an account on the remote computer 
with a username of your choosing. Keep in mind you will need the username to login and access your 
files. Keep it short, simple and alphanumeric.

2. Method of authentification. There are main two methods: passwords and ssh keys. 

2.1. If you are using *passwords* you will be issued a password by your administrator - 
change it on your first login by typing in `passwd` and following the prompt. Ask your administrator 
to reset it if you ever forget it. 

2.2. If the system uses authentication by *ssh keys* you should first generate a public private key pair.
...
There is an alternative to giving your public key: you can simply issue a command:
`ssh-copy-id -i id_rsa.pub your_remote_username@host`
