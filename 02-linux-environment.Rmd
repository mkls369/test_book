...
When working with on larger projects, bigger datasets you will need to work on a unix like system which 
usually means a linux distribution or a version MacOS. Cloud computing services like Amazon's EC2, 
Google's Compute Engine work on linux Virtual Machines so you will have to have some linux knowledge to leverage them.
...
follow along with minimal changes. 
...
First let's get somw terminology out of the way you will often hear these words when referring to the
black (or dark colored) screen with a blinking cursor: terminal(emulator), console, command line, shell. 
Below are definitions from [The Linux Information Project](http://www.linfo.org/) 
...
The original meaning of *console* as it related to computers was (1) an instrument panel containing the controls for a computer ... 
Another term for console as used in this sense is terminal.

These two terms have historical origins - you can read about them in their respective 
[linfo](http://www.linfo.org/)  pages in our case they are often used interchangeably 
to refer to program that **emulates** a machine that is used to input data.
...
So the command line is the textual interface - a place for you to input commands for the computer to execute.
...
One important thing to note is that shells are extremely customizable, we will be using **bash** because of
its popularity and availability.

We will be introducing a more terms along this book, it's a good habit of looking them up to avoid 
confusion.
...
our shell to execute (see it came together!). This section will introduce the main commands you will need in your daily routine. 
Note that you should be careful when not only deleting files, but also writing because your work can be overwritten.

### Navigating folders

I'm assuming you already installed linux or WSL if you are on Windows. The terminal shows where you
are currently you should see a `~$` before your blinking cursor. Type `pwd` for print working directory
...
prompt to do so. It is equivalent to Alt + ↑ or clicking the up arrow in the  File 
...
To do you actual work you first need to connect on a remote server. If your current computer is running
 thus can be done by using [PuTTY](https://www.putty.org/), however I strongly encourage to 
get [WSL](https://docs.microsoft.com/en-us//wsl/install-win10) This way you can enjoy a 
Ubuntu-like terminal on you  machine. This means the following instructions are relevant for both
Windows on WSL and linux users.
...
2.1. If you are using *passwords* you will be issued a password by your administrator - change it on your first login by typing in
